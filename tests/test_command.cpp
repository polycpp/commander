...
TEST(CommandTest, OnCommandFiresAfterSubcommandAction) {
    Command prog("app");
    prog.command("clone <repo>")
        .action([](const auto&, const auto&, auto&) {});

    std::vector<std::string> capturedOperands;
    prog.onCommand("clone", [&](const auto& operands, const auto&) {
        capturedOperands = operands;
    });

    prog.parse({"clone", "git@example.com:foo.git"}, {.from = "user"});
    ASSERT_EQ(capturedOperands.size(), 1u);
    EXPECT_EQ(capturedOperands[0], "git@example.com:foo.git");
}

TEST(CommandTest, OnCommandFiresWithoutActionAsActionSubstitute) {
    Command prog("app");
    prog.command("clone <repo>");  // no .action()
...
TEST(CommandTest, OnAnyCommandFiresOnUnknownSubcommand) {
    Command prog("app");
    prog.command("clone <repo>")
        .action([](const auto&, const auto&, auto&) {});
    ...
    // "fetch" is not registered; command:* listener catches it instead of unknownCommand
    prog.parse({"fetch", "origin"}, {.from = "user"});
    ASSERT_GE(capturedOperands.size(), 1u);
    EXPECT_EQ(capturedOperands[0], "fetch");
}

TEST(CommandTest, OnAnyCommandSuppressesUnknownCommandError) {
    Command prog("app");
    prog.exitOverride();
    prog.command("clone")
        .action([](const auto&, const auto&, auto&) {});
    prog.onAnyCommand([](const auto&, const auto&) {});

    // With the catch-all listener installed, parsing an unknown command
    // must not throw a CommanderError("commander.unknownCommand").
    EXPECT_NO_THROW(prog.parse({"fetch"}, {.from = "user"}));
}

TEST(CommandTest, OnCommandReturnsThis) {
    Command prog("app");
    auto& ref = prog.onCommand("foo", [](const auto&, const auto&) {});
    EXPECT_EQ(&ref, &prog);
    auto& ref2 = prog.onAnyCommand([](const auto&, const auto&) {});
    EXPECT_EQ(&ref2, &prog);
}
